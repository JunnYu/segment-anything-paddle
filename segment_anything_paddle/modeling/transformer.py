...
USE_XFORMERS = hasattr(F, "scaled_dot_product_attention_")
...
class TwoWayTransformer(nn.Layer):
    ...
    def forward(
        self,
        image_embedding: paddle.Tensor,
        image_pe: paddle.Tensor,
        point_embedding: paddle.Tensor,
...
class Attention(nn.Layer):
    ...
    def _separate_heads(
        self, x: paddle.Tensor, num_heads: int, transpose: bool = True
    ) -> paddle.Tensor:
        x = x.reshape([0, 0, num_heads, x.shape[-1] // num_heads])
        if transpose:
            x = x.transpose([0, 2, 1, 3])
        return x  # B x N_heads x N_tokens x C_per_head

    def _recombine_heads(self, x: paddle.Tensor, transpose: bool = True) -> paddle.Tensor:
        if transpose:
            x = x.transpose([0, 2, 1, 3])
        return x.reshape([0, 0, x.shape[2] * x.shape[3]])  # B x N_tokens x C
    ...
    def forward(self, q: paddle.Tensor, k: paddle.Tensor, v: paddle.Tensor) -> paddle.Tensor:
        ...
        # Separate into heads
        q = self._separate_heads(q, self.num_heads, transpose=not USE_XFORMERS)
        k = self._separate_heads(k, self.num_heads, transpose=not USE_XFORMERS)
        v = self._separate_heads(v, self.num_heads, transpose=not USE_XFORMERS)

        if USE_XFORMERS:
            out = F.scaled_dot_product_attention_(q, k, v, attn_mask=None, dropout_p=0.0)
        else:
            # Attention
            _, _, _, c_per_head = q.shape

            attn = paddle.matmul(q, k, transpose_y=True)  # B x N_heads x N_tokens x N_tokens
        ...
        out = self._recombine_heads(out, transpose=not USE_XFORMERS)
